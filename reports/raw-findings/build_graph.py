...
def extend(path):
    """Load extra edges/nodes emitted by later waves (JSON with nodes/edges or add_nodes/add_edges/update_nodes/remove_edges)."""
    p = Path(path)
    if not p.exists(): return
    d = json.loads(p.read_text())
    for n in d.get('nodes', []) + d.get('add_nodes', []): nodes.setdefault(n['id'], n)
    for u in d.get('update_nodes', []):
        if u['id'] in nodes: nodes[u['id']].update(u)
        else: nodes[u['id']] = u
    rm = {(r['source'], r['target'], r.get('type')) for r in d.get('remove_edges', [])}
    if rm:
        edges[:] = [e for e in edges if (e['source'], e['target'], e['type']) not in rm and (e['source'], e['target'], None) not in rm]
    for e in d.get('edges', []) + d.get('add_edges', []): edges.append(e)
...
extend('/private/tmp/claude-502/-Users-rana-singh/cca012eb-0163-4ad1-8765-4fb6bafddfe7/scratchpad/rzp-payouts-architecture/graph_delta.json')

# ---------------- Emit ----------------
OUT.mkdir(parents=True, exist_ok=True)
graph = dict(version='2026-09-04.v2', generated_by='rzp payouts architecture investigation (root agent)', commits=SHA,
