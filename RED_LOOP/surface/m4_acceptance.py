...
class Evaluator:
    ...
    def replay_evidence(self):
        ...
        # G71: final working tree clean (machine). The evaluator's OWN outputs
        # (the acceptance JSON, evidence manifest and its .sha256) are excluded --
        # they are regenerated by this very run and are committed as the evidence
        # commit; everything else must be committed.
        _own = {"reports/implementation/m4-direct-e2e-acceptance.json",
                "reports/implementation/m4-direct-e2e-evidence-manifest.json",
                "reports/implementation/m4-direct-e2e-evidence-manifest.json.sha256"}
        porcelain = git(["status", "--porcelain"])
        _residual = [ln for ln in porcelain.splitlines()
                     if ln[3:].strip() not in _own]
        clean = len(_residual) == 0
