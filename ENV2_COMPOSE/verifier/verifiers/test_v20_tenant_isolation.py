...
def test_other_merchant_cannot_fetch_or_cancel(
    ps_public_client, payouts_mysql, merchant_m1, merchant_m2, ledger_pg
):
    ...
    owner_fetch = ps_public_client.get("/v1/payouts/%s" % m1_payout_id, passport_jwt=m1_jwt)
    assert owner_fetch.status==200,owner_fetch
    assert owner_fetch.json()["id"]==m1_payout_id
    ...
    cancel_resp = ps_public_client.post(
        "/v1/payouts/cancel_payout/%s" % pf.db_id(m1_payout_id), body={}, passport_jwt=m2_jwt
    )
    ...
    for denied in (fetch_resp,cancel_resp):
        assert denied.json()["error"]["description"]=="The id provided does not exist",denied
